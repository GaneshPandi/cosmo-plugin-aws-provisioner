...
def get_server_by_context(ec2_client, ctx):
    ...
    Otherwise, an iteration on all servers will be done for the tag.
    """
...
def get_state(ctx, ec2_client, **kwargs):
    ...
    if str(server_state[0]['Status']) == "running":
        ctx['ip'] = server_state[0]['Public IP']
        # The ip of this instance in the management network
        ctx.logger.info("Server's Public IP {0}".format(ctx['ip']))
        return True
    return False
...
def delete_security_group(ctx, ec2_client, **kwargs):
    ...
    security_group = {}
    security_group.update(copy.deepcopy(ctx.properties['security_group']['delete']))
    ...
    if security_group_presence:
        ...
        raise RuntimeError(
            "Cannot delete Security Group - Security Group doesn't exist {0}"
            .format(security_group['del_sg_name']))
    ...
    if security_group_presence:
        try:
            ec2_client.authorize_security_group(
                ...
                cidr_ip=security_group['cidr_ip'],
        ...
        raise RuntimeError(
            "Unable to Add rules to the Group - Security Group doesn't exist{0}"
            .format(security_group['name']))
